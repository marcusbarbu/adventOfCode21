...
class CaveGraph:
    ...
    def traverse(self, node=None, traversed=set()):
        count = 0
        paths = []
        if node is None:
            node = self.start
        ...
        for neighbor in node.neighbors:
            if neighbor.small and neighbor in traversed or neighbor == self.start:
                continue
            else:
                nc, np = self.traverse(neighbor, {*traversed | set([node])})
                count += nc
                for n in np:
                    paths.append([node] + n)
        return count, paths
